...
import { UserRound, Clock3, Settings2, TrendingUp, MessageCircle, SlidersHorizontal, Flower2, Route, Bookmark, Sun, Moon, ArrowLeft, ArrowUpRight, Headphones, Sparkles, Volume2, VolumeX, Bell, BookOpen, ChevronRight, Heart, LogOut, Pause, Play, Search, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  if (stage === 'app' && mainMenu) {
    return <main className="app-shell app-main section-app day-one-screen welcome-carousel-screen"><section className="day-one-section"><header className="assistant-welcome"><p>Hola, ¿cómo estás, {userName}?</p><h1>Bienvenido al<strong>Asistente de Germán</strong></h1></header><DayOneCarousel label="Secciones del Asistente de Germán" items={mainCategories.map(([target, , title, detail]) => ({ target, title, detail, image: target === 'espacio' ? '/mi-perfil-mujer-movil-serena.png' : target === 'notificaciones' ? '/notificaciones.png' : target === 'biblioteca' ? '/biblioteca-lectora.png' : target === 'talleres' ? '/practicas-guiadas-hombre.png' : target === 'propia' ? '/tu-propia-practica-mujer.png' : target === 'meditaciones' ? '/meditaciones-hombre.png' : target === 'consultas' ? '/consultas-mujer.png' : undefined, imageSize: ['espacio', 'biblioteca', 'talleres', 'propia'].includes(target) ? 'compact' as const : undefined }))} onSelect={(item) => { setTab(item.target); setTrail([]); setReader(null); setMainMenu(false); }} /></section></main>;
  }
  ...
  return <main className="app-shell app-main section-app day-one-screen"><section className="day-one-section"><FixedHeader eyebrow={screen.eyebrow} title={screen.title} subtitle={screen.subtitle} onBack={back} /><DayOneCarousel key={`${tab}-${trail.map((item) => item.title).join('/')}`} label={screen.title} items={screen.items.map((item) => item.accountPanel ? { ...item, image: '/mi-cuenta-acceso.png' } : item.title === 'Favoritos' ? { ...item, image: '/favoritos-guardados.png' } : item)} onSelect={select} isFavorite={(item) => item.reader ? favorites.some((favorite) => favorite.id === deckFavorite(item).id) : undefined} onFavorite={(item) => toggleFavorite(deckFavorite(item))} /></section></main>;
}
