...
import { authenticateRequest, findPaymentById, findPaymentByOrder } from '../../../../lib/server/payment-server';
...
export async function POST(request: Request) {
  ...
  if (!body.token) return NextResponse.json({ error: 'Falta la referencia del pago.' }, { status: 400 });
  try {
    const payment = body.payment ? await findPaymentById(body.payment, 'paypal') : await findPaymentByOrder('paypal', body.token);
