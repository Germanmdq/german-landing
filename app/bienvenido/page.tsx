...
type Step='video'|'install'|'login'|'activate';
...
export default function Bienvenido(){
 ...
 useEffect(()=>{supabase.auth.getSession().then(({data})=>{setSession(data.session);setChecked(true)});const {data}=supabase.auth.onAuthStateChange((_e,s)=>{setSession(s);setChecked(true);if(s&&step==='login')setStep('activate')});return()=>data.subscription.unsubscribe()},[step]);
 ...
 if(step==='video')return <Intro onFinish={()=>setStep('install')}/>;
 if(step==='install')return <InstallOnboarding suggestedPlatform={platform} nativePromptAvailable={prompt} onClose={()=>setStep('login')} onInstallAndroid={promptNativeInstallation} onRecheckInstallation={()=>{const v=isRunningStandalone();setStandalone(v);return v}}/>;
 if(!checked)return null;
 if(step==='login'&&!session)return <LoginGate redirectPath="/bienvenido?paso=activar"/>;
 if(step==='login'&&session){queueMicrotask(()=>setStep('activate'));return null}
 if(!session)return <LoginGate redirectPath="/bienvenido?paso=activar"/>;
